...
const SideBar = ({ sidebarOpen, setSidebarOpen, sidebarData }: ISideBarProps) => {
  ...
  const _makeGroup = () => {
    ...
    return sidebarData.map((sb) => {
      if (!sb.children?.length) {

        return (
          <li className={`px-3 py-2 rounded-sm mb-0.5 last:mb-0 ${getAsPath.includes(sb.path) && "bg-slate-900"}`}>
            <Link

              to={sb.path}
              className={`block text-slate-200 truncate transition duration-150 ${getAsPath.includes(sb.path) ? "hover:text-slate-200" : "hover:text-white"
                }`}
            >
      ...
      return (
        <li className="mb-1 last:mb-0">
          <Link
            ...
            to={item.path}
            className={`block transition duration-150 truncate ${isActive ? " text-indigo-500" : ""}`}
          >
